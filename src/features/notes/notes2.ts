...
export function makeOrdable(target, cb) {
...
export async function updateReorderdList() {
    const notes = useNotesStore.getState().notes;
    let newNotes: TNote[] = [];

    const notesListDiv = document?.getElementById("list");

    notesListDiv?.querySelectorAll("li").forEach(async (li) => {
        const note = notes.filter((i) => i._id === li.id);
        newNotes = newNotes.concat(note);

        if (newNotes.length === notes.length) useNotesStore.getState().resetNotes(newNotes);
    });
}
